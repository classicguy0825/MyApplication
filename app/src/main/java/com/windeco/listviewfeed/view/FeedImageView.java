package com.windeco.listviewfeed.view;
...
public class FeedImageView extends ImageView {
	...
	private static int mFeedImageViewHeight = 0 ;
	...
	public void setImageUrl(String url, ImageLoader imageLoader) {
		...
		Log.d("로그", " FeedImageView's mUrl : "+ mUrl);
		Log.d("로그", " FeedImageView's imageLoader : "+ imageLoader);
	...
	private void loadImageIfNecessary(final boolean isInLayoutPass) {
		...
		Log.d("로그", "FeedImageView 1");

		// if the view's bounds aren't known yet, and this is not a
		// wrap-content/wrap-content
		// view, hold off on loading the image.

		Log.d("로그", "getWidth / getHeight() : " + getWidth() + " / " + getHeight());

		if (width == 0 && height == 0 && !isFullyWrapContent) {

			Log.d("로그", "isFullyWrapContent : " + isFullyWrapContent);
			return;
		}

		Log.d("로그", " FeedImageView 2");
		...
		ImageContainer newContainer = mImageLoader.get(mUrl, new ImageListener() {
					...
					public void onResponse(final ImageContainer response, boolean isImmediate) {
						...
						if (response.getBitmap() != null) {
							...
							Log.d("로그", " FeedImageView's getBitmap : "+ response.getBitmap());
							bWidth = response.getBitmap().getWidth();
							bHeight = response.getBitmap().getHeight();

							mFeedImageViewHeight = mFeedImageViewHeight + bHeight;

							resizeParentLayoutHeight(mFeedImageViewHeight);
	...
	private void resizeParentLayoutHeight(int height) {
		...
		if (layout != null) {
			...
			if (params != null) {
				...
				Log.d("로그", " New resizeParentLayoutHeight : "+ height);
				post(new Runnable() {
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		Log.d("로그", " FeedImageView's onLayout ");
 	}
